...
aws.bucket.name=cs6240-hw4-bucket-pritish
aws.input=input
aws.output=output
aws.log.dir=log
...
local: jar clean-local-output
	spark-submit --class ${job.name} --master ${local.master} --name "${app.name}" ${jar.name} ${local.k} ${local.numOfIterations} ${local.input} ${local.output}
...
aws: jar upload-app-aws delete-output-aws
	aws emr create-cluster \
		--name "PageRank Spark Cluster (k=10k, i=3)" \
		...
		--steps Type=CUSTOM_JAR,Name="${app.name}",Jar="command-runner.jar",ActionOnFailure=TERMINATE_CLUSTER,Args=["spark-submit","--deploy-mode","cluster","--class","${job.name}","s3://${aws.bucket.name}/${jar.name}","${local.k}","${local.numOfIterations}","s3://${aws.bucket.name}/${aws.input}","s3://${aws.bucket.name}/${aws.output}"] \
...
download-log-aws:
	mkdir spark_logs_aws
	aws s3 sync s3://${aws.bucket.name}/${aws.log.dir} spark_logs_aws
